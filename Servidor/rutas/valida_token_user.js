...
const valida_token = async function (req, res, next) {
    ...
    .then((sql) => {
        if (sql[0] == '' || sql[0] == null) {
            ...
            if(typeof barerHeader != 'undefined'){
                ...
                res.sendStatus(403).send('Peticion no autorizada'); next('error');
            }
        
            jwt.verify(req.token, process.env.FIRMA, (err, data) => {
                if (err){ res.status(403).send('Certificado erroneo!'); next('error')}
                else{if(sql[0].nombre_usuario != data.user) {res.status(403).send('Usuario diferente al del token, por favor verifique!'); next('error')}}
            })
